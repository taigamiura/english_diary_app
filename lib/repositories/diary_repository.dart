...
class DiaryRepository {
  ...
  Future<List<Diary>> fetchDiaries({
    ...
    return logRequestResponse('DiaryRepository.fetchDiaries',
      () async {
  ...
  Future<List<Diary>> fetchDateDiary({
    ...
    required DateTime date,
      ...
      logRequestResponse('DiaryRepository.fetchDateDiary',
        () async {
          final query = apiRepository.client
          .from(diaryTable)
          .select()
          .eq('user_id', userId)
          .filter('created_at', 'eq', date.toIso8601String().substring(0, 10)) // 'YYYY-MM-DD'
          .limit(1);
          final result = await query;
          return List<Map<String, dynamic>>.from(result)
          .map((json) => Diary.fromJson(json))
  ...
  Future<List<String>> fetchPostedMonths(String userId) =>
      logRequestResponse('DiaryRepository.fetchPostedMonths',
        () async {
          final result = await apiRepository.client
            .rpc('get_posted_months', params: {'user_id_param': userId});
  ...
  Future<Diary?> fetchDiary(String id) =>
      logRequestResponse('DiaryRepository.fetchDiary',
        () async {
  ...
  Future<int> fetchAverageTextInput({required String userId}) =>
      logRequestResponse('DiaryRepository.fetchAverageTextInput',
        () async {
          final result = await apiRepository.client
              .rpc('avg_text_length', params: {'user_id_param': userId});
  ...
  Future<void> insertDiary(Diary diary) =>
      logRequestResponseVoid('DiaryRepository.insertDiary',
        () async {
  ...
  Future<void> insertDiaries(List<Diary> diaries) =>
      logRequestResponseVoid('DiaryRepository.insertDiaries',
        () async {
          await apiRepository.insertMany(table: diaryTable, data: diaries.map((d) => d.toJson()).toList());
        },
  ...
  Future<void> updateDiary(Diary diary) =>
      logRequestResponseVoid('DiaryRepository.updateDiary',
        () async {
          await apiRepository.updateOne(table: diaryTable, id: diary.id.toString(), data: diary.toJson());
        },
  ...
  Future<void> updateDiaries(List<Diary> diaries, {String idColumn = 'id'}) =>
      logRequestResponseVoid('DiaryRepository.updateDiaries',
        () async {
          await apiRepository.updateMany(table: diaryTable, data: diaries.map((d) => d.toJson()).toList(), idColumn: idColumn);
        },
        requestDetail: 'count: ${diaries.length}',
      );
  ...
  Future<void> deleteDiary(String id) =>
      logRequestResponseVoid('DiaryRepository.deleteDiary',
        () async {
  ...
  Future<void> deleteDiaries(List<String> ids, {String idColumn = 'id'}) =>
      logRequestResponseVoid('DiaryRepository.deleteDiaries',
        () async {
          await apiRepository.deleteMany(table: diaryTable, ids: ids, idColumn: idColumn);
        },
