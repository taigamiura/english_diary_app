...
class DiaryFeedbackRepository {
  ...
  DiaryFeedbackRepository({ApiRepository? apiRepository}) : apiRepository = apiRepository ?? ApiRepository();

  Future<List<DiaryFeedback>> fetchDiaryFeedbacks({required String diaryEntryId}) =>
      logRequestResponse('DiaryFeedbackRepository.fetchDiaryFeedbacks',
        () async {
          var query = apiRepository.client.from(table).select();
          query = query.eq('diary_entry_id', diaryEntryId);
          final result = await query;
          return List<Map<String, dynamic>>.from(result)
              .map((json) => DiaryFeedback.fromJson(json))
              .toList();
        },
  ...
  Future<DiaryFeedback?> fetchDiaryFeedback(String id) =>
      logRequestResponse('DiaryFeedbackRepository.fetchDiaryFeedback',
        () async {
  ...
  Future<void> insertDiaryFeedback(DiaryFeedback feedback) =>
      logRequestResponseVoid('DiaryFeedbackRepository.insertDiaryFeedback',
        () async {
  ...
  Future<void> updateDiaryFeedback(String id, DiaryFeedback feedback) =>
      logRequestResponseVoid('DiaryFeedbackRepository.updateDiaryFeedback',
        () async {
          await apiRepository.updateOne(table: table, id: id, data: feedback.toJson());
        },
        requestDetail: 'id: $id',
      );

  Future<void> deleteDiaryFeedback(String id) =>
      logRequestResponseVoid('DiaryFeedbackRepository.deleteDiaryFeedback',
        () async {
