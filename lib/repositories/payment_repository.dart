...
class PaymentRepository {
  ...
  PaymentRepository({ApiRepository? apiRepository})
    : apiRepository = apiRepository ?? ApiRepository();

  Future<List<Payment>> fetchPayments({required String profileId}) =>
      logRequestResponse('PaymentRepository.fetchPayments', () async {
        var query = apiRepository.client.from(table).select();
        query = query.eq('user_id', profileId);
        final result = await query;
        return List<Map<String, dynamic>>.from(
          result,
        ).map((json) => Payment.fromJson(json)).toList();
      }, requestDetail: 'profileId: $profileId');

  Future<Payment?> fetchPayment(int id) =>
      logRequestResponse('PaymentRepository.fetchPayment', () async {
        final json = await apiRepository.fetchOne(
          table: table,
          id: id.toString(),
        );
  ...
  Future<void> insertPayment(Payment payment) =>
      logRequestResponseVoid('PaymentRepository.insertPayment', () async {
  ...
  Future<void> updatePayment(int id, Payment payment) =>
      logRequestResponseVoid('PaymentRepository.updatePayment', () async {
  ...
  Future<void> deletePayment(int id) =>
      logRequestResponseVoid('PaymentRepository.deletePayment', () async {
