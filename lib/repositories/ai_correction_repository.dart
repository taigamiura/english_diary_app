...
class AiCorrectionRepository {
  ...
  AiCorrectionRepository({ApiRepository? apiRepository}) : apiRepository = apiRepository ?? ApiRepository();

  Future<List<AiCorrection>> fetchAiCorrections({required String diaryEntryId}) =>
      logRequestResponse('AiCorrectionRepository.fetchAiCorrections',
        () async {
          var query = apiRepository.client.from(table).select();
          query = query.eq('diary_entry_id', diaryEntryId);
          final result = await query;
          return List<Map<String, dynamic>>.from(result)
              .map((json) => AiCorrection.fromJson(json))
              .toList();
        },
  ...
  Future<AiCorrection?> fetchAiCorrection(String id) =>
      logRequestResponse('AiCorrectionRepository.fetchAiCorrection',
        () async {
  ...
  Future<void> insertAiCorrection(AiCorrection correction) =>
      logRequestResponseVoid('AiCorrectionRepository.insertAiCorrection',
        () async {
          await apiRepository.insertOne(table: table, data: correction.toJson());
        },
        requestDetail: 'id: ${correction.id}',
      );

  Future<void> updateAiCorrection(String id, AiCorrection correction) =>
      logRequestResponseVoid('AiCorrectionRepository.updateAiCorrection',
        () async {
          await apiRepository.updateOne(table: table, id: id, data: correction.toJson());
        },
        requestDetail: 'id: $id',
      );

  Future<void> deleteAiCorrection(String id) =>
      logRequestResponseVoid('AiCorrectionRepository.deleteAiCorrection',
        () async {
