...
import 'package:kiwi/views/error_page.dart';
...
Future<void> main() async {
  ...
  try {
    ...
    if (!kDebugMode) {
      ...
      await SentryFlutter.init(
        (options) {
          ...
          options.debug = false;
          options.release = 'KIWI@${dotenv.env['APP_VERSION'] ?? 'unknown'}';
          options.environment = dotenv.env['APP_ENV'] ?? 'development';
          options.enableUserInteractionBreadcrumbs = true;
          options.maxBreadcrumbs = 100;
        },
        appRunner:
            () => runApp(ProviderScope(child: SentryWidget(child: MyApp()))),
      );
    } else {
    ...
    if (dotenv.env['SENTRY_DSN'] != null &&
        dotenv.env['SENTRY_DSN']!.isNotEmpty) {
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: WelcomePage(),
      routes: {'/diaryList': (context) => const DiaryIndexPage()},
    );
  }
}
