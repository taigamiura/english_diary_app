...
class DiaryServiceImpl implements DiaryService {
  ...
  Future<List<Diary>> fetchDiaries({
    ...
    required int limit,
      ...
      repository.fetchDiaries(userId: userId, from: from, to: to, limit: limit);
  ...
  Future<List<YearMonth>> fetchPostedMonths({required String userId}) async {
    ...
    return months.map((e) => YearMonth.fromString(e)).toList()
  ...
  @override
  Future<int> fetchAverageTextInput({required String userId}) =>
      repository.fetchAverageTextInput(userId: userId);
