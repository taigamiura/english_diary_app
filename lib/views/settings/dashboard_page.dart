...
class _DashboardPageState extends ConsumerState<DashboardPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      ref.read(diaryListProvider.notifier).fetchDiaries(userId: userId.toString(), limit: 0);
      ref.read(diaryListProvider.notifier).fetchAverageTextInput(userId: userId.toString());
    });
  }
  @override
  void dispose() {
    ...
    return Scaffold(
      ...
      child: Padding(
        ...
        children: [
                ...
                children: [
                  Row(
                    children: [
                      ...
                      const Text('会員プラン', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                    ],
                  ),
                  const SizedBox(height: 12),
                  const Text('無料プラン: 日記投稿・閲覧・音声録音利用', style: TextStyle(fontSize: 14)),
                  const SizedBox(height: 4),
                  const Text('プレミアム: AI添削・音声再生使い放題', style: TextStyle(fontSize: 14, color: AppColors.mainColor)),
                  const SizedBox(height: 12),
                  Align(
          ...
          Row(
            children: [
              Expanded(child: SizedBox(height: 100, child: _buildStreakCard(streak))),
              const SizedBox(width: 16),
              Expanded(child: SizedBox(height: 100, child: _buildAvgLengthCard(diaries, averageTextInput))),
            ],
          ),
          const SizedBox(height: 12),
          // おすすめコンテンツ例
          Card(
            ...
            child: Padding(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Text('おすすめ', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                  const SizedBox(height: 8),
                  ListTile(
                    leading: const Icon(Icons.lightbulb, color: AppColors.mainColor),
  ...
  int _calcStreak(List<DateTime> postedDates) {
    ...
    while (uniqueDates.isNotEmpty && uniqueDates.last == DateTime(day.year, day.month, day.day)) {
      streak++;
              ...
              child: Column(
                ...
                children: [
                  FittedBox(
                    ...
                    child: const Text('連続投稿日数', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                  ),
                  FittedBox(
                    fit: BoxFit.scaleDown,
                    alignment: Alignment.centerLeft,
                    child: Text('$streak 日', style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: AppColors.mainColor)),
                  ),
                ],
              ),
            ),
              ...
              child: Column(
                ...
                children: [
                  FittedBox(
                    fit: BoxFit.scaleDown,
                    alignment: Alignment.centerLeft,
                    child: const Text('平均投稿文字数', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                  ),
                  FittedBox(
                    fit: BoxFit.scaleDown,
                    alignment: Alignment.centerLeft,
                    child: Text(
                      '$averageTextInput 文字',
                      style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: AppColors.mainColor),
                    ),
                  ),
                ],
              ),
